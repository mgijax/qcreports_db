...
def go (form) :
    ...
    results = db.sql('''
        ...
        from GXD_HTSample ht, GXD_Genotype g, GXD_AlleleGenotype ga, ALL_Allele aa, ACC_Accession a, PRB_Strain s, ACC_Accession sa
        ...
        select * from alleles
        union
        select distinct a.accid, aa._allele_key, aa.symbol, null, 'no' as hasJR, 
            string_agg(distinct s.strain, '|') as strains
        from GXD_HTSample ht, GXD_Genotype g, GXD_AlleleGenotype ga, ALL_Allele aa, ACC_Accession a, PRB_Strain s
        where ht._genotype_key = g._genotype_key
        and g._genotype_key = ga._genotype_key
        and ga._allele_key = aa._allele_key
        and aa.iswildtype = 0
