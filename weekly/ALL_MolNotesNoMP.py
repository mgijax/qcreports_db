...
fp.write(str.ljust('Approval', 15) + \
         str.ljust('Acc ID', 15) + \
         str.ljust('Allele Type', 45) + \
         str.ljust('Symbol', 45) + \
         str.ljust('References', 15) + CRT*2)
...
for r in results:
    ...
    if r['_Allele_key'] not in refs:
        skipped += 1
        continue

    fp.write(str.ljust(r['cDate'], 15) + \
             str.ljust(r['accID'], 15) + \
             str.ljust(r['alleleType'], 45) + \
             str.ljust(r['symbol'], 45))

    # total number of references associated with each alleles
    fp.write(str.ljust(str(len(refs[r['_Allele_key']])), 15))
    fp.write(','.join(refs[r['_Allele_key']]) + CRT)
