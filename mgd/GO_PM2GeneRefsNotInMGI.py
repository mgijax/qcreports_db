...
results = db.sql('''select a.accid as egID
...
results = db.sql('''select distinct aa.accid as egID
    ...
    and e._evidenceterm_key not in (115, 3251466, 7428292)''', 'auto')
    # IEA, ISO, IBA

for r in results:
    mrkGOAnnotList.append(r['egID'])

db.sql('''select a1._Object_key as refsKey, a1.accid as mgiID,
         a3.accid as pubmedid
