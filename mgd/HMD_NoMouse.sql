...
select distinct h._Class_key, h.symbol, m.symbol "mouse symbol", organism = h.commonName, h.jnumID
from #homology h, MRK_Marker m
where h.symbol not like '*%'
and h.symbol *= m.symbol
and m._Organism_key = 1
union
select distinct h._Class_key, h.symbol, m.symbol, organism = h.commonName, h.jnumID
from #homology h, MRK_Marker m
where h.symbol like '*%'
