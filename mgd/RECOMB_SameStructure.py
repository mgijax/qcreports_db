...
db.sql('''
	select gr._Specimen_key, grs._Structure_key 
    	into #specstructs 
...
db.sql('''
	select * into #dupspecimens 
    	from #specstructs 
    	group by _Specimen_key, _Structure_key having count(*) > 1
	''', None)
...
db.sql('''
	select distinct ss.*, gs.specimenLabel, ga._Refs_key, gs._Assay_key, gsn.structure 
    	into #specimens 
    	...
    	and s._StructureName_key = gsn._StructureName_key
	''', None)

# get the all gel/structure pairs

db.sql('''
	select gr._GelLane_key, grs._Structure_key 
    	into #gelstructs 
...
db.sql('''
	select * into #dupgels 
    	from #gelstructs 
    	group by _GelLane_key, _Structure_key having count(*) > 1
	''', None)
...
db.sql('''
	select distinct ss.*, gs.laneLabel, ga._Refs_key, gs._Assay_key, gsn.structure 
    	into #gels 
...
db.sql('''
	select distinct ss.specimenLabel, ss.structure, a1.accid as mgiID, a2.accid as jnumID, a2.numericPart 
    	into #finalSpecimen 
...
db.sql('''
	select distinct ss.laneLabel, ss.structure, a1.accid as mgiID, a2.accid as jnumID, a2.numericPart 
    	into #finalGel 
...
results = db.sql('''
	(
	select specimenLabel, structure, mgiID, jnumID, numericPart from #finalSpecimen 
	union 
