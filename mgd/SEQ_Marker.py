...
db.sql('select s._Sequence_key, s._Marker_key, s.accID ' + \
	'into #seqMarker1 ' + \
...
db.sql('create index idx_key3 on #seqMarker1(accID)', None)

db.sql('select distinct _Sequence_key, _Marker_key, accID into #seqMarker from #seqMarker1', None)
...
db.sql('select distinct _Sequence_key, accID into #uniqueSeq from #seqMarker', None)
...
db.sql('select a.accID, a._Sequence_key, sq.length, sq.description, ' + \
	'sq._SequenceType_key, sq._SequenceProvider_key ' + \
	'into #seqAttr ' + \
	'from #uniqueSeq a, SEQ_Sequence sq ' + \
	'where a._Sequence_key = sq._Sequence_key ', None)
