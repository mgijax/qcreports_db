...
#
db.sql('select _Marker_key, idx_count = count(*) ' + \
       'into #counts ' + \
       'from GXD_Index ' + \
       'group by _Marker_key ' + \
       'having count(*) >= 40', None)

#
# Get the marker and reference keys for each gene identified in the
# first query.
#
db.sql('select _Marker_key, _Refs_key ' + \
       'into #markerref ' + \
       'from GXD_Index gi ' + \
       'where exists (select 1 from #counts c ' + \
                     'where gi._Marker_key = c._Marker_key)', None)
...
#
db.sql('select mr._Marker_key, giv.symbol, giv.jnumID, ' + \
              'gi._Priority_key, vt.term, giv.short_citation, ' + \
              'sort_field = str(br.year) + " " + giv.jnumID ' + \
       'into #details ' + \
       ...
       'where mr._Marker_key = gi._Marker_key and ' + \
             ...
             'not exists (select 1 from GXD_Assay ga ' + \
                         'where mr._Marker_key = ga._Marker_key and ' + \
                               'mr._Refs_key = ga._Refs_key)', None)
...
#
db.sql('select mr._Marker_key, ' + \
              'sort_field = min(str(br.year) + " " + bcc.jnumID) ' + \
       'into #firstref ' + \
...
results = db.sql('select d.symbol, c.idx_count, d.jnumID, d.term, ' + \
                 ...
                 'order by d._Priority_key, c.idx_count desc', 'auto')
