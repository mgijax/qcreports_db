'''
   Papers not in public MGI for genes with no MP annotations

    * Mouse
    * Do not have a J number assignment in MGI (ref by PMID may or may not be in MGI)
    * Do not have a lit triage status of “rejected” for AP
...
fp.write('Papers not in public MGI for genes with no MP annotations\n\n')
fp.write('    * Mouse\n')
fp.write('    * Have <= 15 egIDs per PubMed ID\n')
fp.write('    * Do not have a J number assignment in MGI (ref by PMID may or may not be in MGI)\n')
fp.write('    * Do not have a lit triage status of “rejected” for AP\n')
...
db.sql('''
    ...
    union
    select geneid, pubmedid, null
    from DP_EntrezGene_PubMed eg
    where eg.taxid = 10090
    and not exists (select 1 from BIB_Citation_Cache c where eg.pubmedid = c.pubmedid)
    ''', None) 
