...
db.sql('''select v.*, a1.creation_date as genoCreateDate, a1.accid as genoAccid, c1.cellLine as mcl1, g.isConditional
    into temporary table allelePair1
    from GXD_AllelePair_View v, ACC_Accession a1, ALL_CellLine c1, GXD_Genotype g
    where v._MutantCellLine_key_1 = c1._CellLine_key
    and v._Genotype_key = a1._Object_key
    and a1._MGIType_key = 12
    and a1._LogicalDB_key = 1
    and v._Genotype_key = g._Genotype_key''', None)

db.sql('''create index idx1 on allelePair1(_MutantCellLine_key_2)''', None)
...
db.sql('''create index idx2 on allelePair2(_Genotype_key)''', None)

results = db.sql('''select distinct a.*, s._Strain_key, s.strain
    from allelePair2 a, GXD_Genotype g, PRB_Strain s
...
fp.write('genotypeIds%ssymbol%sallele1%sallele2%salleleState%smcl1%smcl2%sstrain%sisConditional%s' % (TAB, TAB, TAB, TAB, TAB, TAB, TAB, TAB, CRT))
...
for gId in genoDict:
	...
	k = '%s|%s|%s|%s|%s|%s|%s|%s' %(symbol, allele1, allele2, alleleState, mcl1, mcl2, strain, isConditional)
	keyList.append(k)
